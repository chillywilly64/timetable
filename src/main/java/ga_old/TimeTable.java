...
public class TimeTable {
	private List<ClassRoom> rooms = new ArrayList<ClassRoom>();
	private int fittness;
	private List<Lecture> classes=new ArrayList<>();
	private List<StudentGroups> studentGroups=new ArrayList<>();
	private List<ClassRoom> practicalRooms = new ArrayList<ClassRoom>();
	private List<ClassRoom> theoryRooms = new ArrayList<ClassRoom>();
	private List<StudentGroups> theoryStudentGroups=new ArrayList<>();
	private List<StudentGroups> practicalStudentGroups=new ArrayList<>();
	private Map<Combination, Week> personalTimeTable= new HashMap<Combination, Week>();
	//private ArrayList<Professor> professors=new ArrayList<>();
	//adds more rooms to timetable

	public TimeTable(List<ClassRoom> classroom, List<Lecture> lectures){//, ArrayList<Professor> professors){
...
//	}
	...
	public void setTimeTable(List<StudentGroups> studentGroups2, List<ClassRoom> rooms2, String string) {
		// TODO Auto-generated method stub
	...
	private ClassRoom getBestRoom(int size, List<ClassRoom> rooms2) {
		...
		int delta = 1000;
		ClassRoom room = null;
		for (ClassRoom tempRoom: rooms2){
			System.out.println(tempRoom.getRoomNo() + " " + rooms2.size());
			System.out.println(!checkOccupiedRoom(tempRoom, rooms2));
			if(!checkOccupiedRoom(tempRoom, rooms2)){
		        int tmp = Math.abs(size - tempRoom.getSize());
				System.out.println(tmp + " " + delta);
				if(tmp < delta){
		            delta = tmp;
		            room = tempRoom;
		    }
